...
case class RenameProperties(state: collection.Map[String, String])
  extends StatefulTransformer[collection.Map[String, String]] {

  override def apply(q: Ast): (Ast, StatefulTransformer[collection.Map[String, String]]) =
    q match {
      case Property(a, b) if (state.contains(b)) =>
        (Property(a, state(b)), this)
      case e: Entity =>
        (e, RenameProperties(e.properties.map(a => a.property -> a.alias).toMap))
      case other => 
        super.apply(other)
    }
}
